...
FROM
   transactions 
   JOIN
      marketing_campaigns USING(product_id) 
WHERE
...
(
   SELECT
      ...
      RANK() OVER(
   ORDER BY
      SUM(quantity) DESC) rankk 
   FROM
      transactions 
      JOIN
         sustainable_clothing USING(product_id) 
   GROUP BY
      product_name 
   ORDER BY
      quantity_sold DESC
...
(
   SELECT
      ...
      RANK() OVER(
   ORDER BY
      round(SUM(quantity*price), 2) DESC) AS rankk 
   FROM
      transactions 
      JOIN
         sustainable_clothing USING(product_id) 
...
SELECT
   campaign_name,
   round(SUM(quantity*price) / 30, 2) AS avg_revenue_per_day 
FROM
   transactions 
   JOIN
      marketing_campaigns USING(product_id) 
...
SELECT
   *,
   concat(round((revenue / total_revenue)*100, 2), '%') AS product_contribution 
FROM
   cte,
   cte2 
ORDER BY
   product_contribution DESC;
-----------------------
...
(
   SELECT
      product_name,
      round(AVG(quantity), 2) AS avg_quantity_during_campaign 
   FROM
      transactions 
      JOIN
         marketing_campaigns USING(product_id) 
...
(
   SELECT
      product_name,
      round(AVG(quantity), 2) AS avg_quantity_outside_campaign 
   FROM
      transactions 
      JOIN
         marketing_campaigns USING(product_id) 
      JOIN
         sustainable_clothing USING(product_id) 
   WHERE
      purchase_date NOT BETWEEN start_date AND end_date 
   GROUP BY
      product_name
)
SELECT
   cte.product_name,
   avg_quantity_during_campaign,
   avg_quantity_outside_campaign 
FROM
   cte 
   JOIN
      cte2 USING(product_name) ;
-----------------------
...
(
   SELECT
      product_name,
      round(SUM(quantity*price), 2) AS revenue_during_campaign 
   FROM
      transactions 
      JOIN
         marketing_campaigns USING(product_id) 
...
(
   SELECT
      product_name,
      round(SUM(quantity*price), 2) AS revenue_outside_campaign 
   FROM
      transactions 
      JOIN
         marketing_campaigns USING(product_id) 
...
SELECT
   ...
   DENSE_RANK() OVER(
ORDER BY
   AVG(quantity) DESC) AS product_rank 
FROM
   transactions 
   JOIN
      sustainable_clothing USING(product_id) 
